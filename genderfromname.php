...
function gender_init($females_ref=null, $males_ref=null) {

    global $Males;
    global $Females;

    if (!$females_ref || !$males_ref) {
        // Do nothing, the arrays are pre-populated at the end of the file
    }
    else if ($males_ref && !$females_ref) {
        error_log("Male hash supplied, but not female!");
    }
    else if ($females_ref && !$males_ref) {
        error_log("Female hash supplied, but not male!");
    }
    else {
        $Males = arsort($males_ref);
        $Females = arsort($females_ref);
    }
}

function gender($name, $looseness=1) {
    global $DEBUG;
    global $DEBUG_MSG;
    global $MATCH_LIST;
